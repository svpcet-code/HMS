...
export function RemoveBedModal({ beds }: { beds: { id: string, bedNumber: string, ward: string }[] }) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        render={
          <Button variant="outline" className="w-full sm:w-auto border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 font-bold transition-all shadow-sm">
            <Trash2 className="mr-2 h-4 w-4" /> Finalize Deletions
          </Button>
        }
      />
      <DialogContent className="sm:max-w-[425px] rounded-3xl border-none shadow-2xl p-0 overflow-hidden">
        <DialogHeader className="bg-red-50 p-6 border-b border-red-100 text-center">
            ...
            <DialogTitle className="text-xl font-bold text-red-900">Decommission Bed</DialogTitle>
            <p className="text-xs text-red-600 mt-1 font-medium">This action cannot be undone if the bed has no active patients.</p>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div className="space-y-2">
            <Label htmlFor="bedId" className="text-slate-500 text-xs font-bold uppercase tracking-wider">Select Bed to Remove</Label>
            <Select name="bedId" required>
              <SelectTrigger className="py-7 rounded-2xl border-slate-200 bg-slate-50/50 font-semibold">
                <SelectValue placeholder="Choose a bed identifier..." />
              </SelectTrigger>
              <SelectContent className="rounded-xl p-1 max-h-[300px]">
                {beds.map((b) => (
                  <SelectItem key={b.id} value={b.id} className="rounded-lg py-3 cursor-pointer">
                    <div className="flex flex-col">
                        <span className="font-bold">{b.bedNumber}</span>
                        <span className="text-[10px] text-slate-500 uppercase">{b.ward}</span>
                    </div>
                  </SelectItem>
                ))}
              </SelectContent>
